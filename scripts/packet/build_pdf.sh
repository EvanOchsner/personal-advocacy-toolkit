...
#
# This is a thin wrapper around `python -m scripts.packet.build`. It
...
#      parent's parent) so `python -m scripts.packet.build` works
...
Usage: build_pdf.sh MANIFEST [BUILD_ARGS...]
  ...
  BUILD_ARGS    Forwarded verbatim to `python -m scripts.packet.build`.
